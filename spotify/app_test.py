...
def test_client():
    ...
    flask_app.config["DATABASE"] = "sqlite:///:memory:"  # Use an in-memory DB for testing
...
def test_index_route(test_client):
    ...
    assert b"Home" in response.data  # Update this string to match the actual content of your index.html
...
def test_query_route_valid_artist_mocked(test_client, monkeypatch):
    ...
    monkeypatch.setattr("spotify.APIQueries.get_top_tracks", mock_get_top_tracks)

    data = {"spotify_artist1": "Mock Artist", "spotify_artist2": "", "spotify_artist3": ""}
...
def test_query_route_with_real_artist(test_client):
    ...
    # Use a real artist name (e.g., Taylor Swift)
    data = {"spotify_artist1": "Taylor Swift", "spotify_artist2": "", "spotify_artist3": ""}

    # Send POST request to /query route
    response = test_client.post("/query", data=data)

    assert response.status_code == 200  # Ensure the response is successful
    assert b"Taylor Swift" in response.data  # Check if the artist name is in the response
...
def test_save_tracks_route(test_client, monkeypatch):
    ...
    monkeypatch.setattr("spotify.APIQueries.audio_features", mock_audio_features)
    monkeypatch.setattr("spotify.databases.insert_song", mock_insert_song)
...
def test_view_playlist_route_mocked(test_client, monkeypatch):
    ...
    def mock_fetch_all_songs():
        return [
            {"id": "track1", "name": "Track 1", "artist": "Artist 1", "danceability": 0.9},
            {"id": "track2", "name": "Track 2", "artist": "Artist 2", "danceability": 0.7},
        ]

    monkeypatch.setattr("spotify.databases.fetch_all_songs", mock_fetch_all_songs)
...
def test_view_playlist_with_real_data(test_client):
    ...
    assert response.status_code == 200
    # Verify the playlist has content (adjust based on your app's behavior)
